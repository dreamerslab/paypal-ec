 ...
 */
PayPalEC.prototype.make_payment_url = function ( token, path ){
  return url.format({
    protocol : 'https:',
    host     : ( this.sandbox ) ? SANDBOX_URL : REGULAR_URL,
    pathname : path !== undefined ? path: '/cgi-bin/webscr',
    query    : {
 ...
 */
PayPalEC.prototype.set = function ( params, path , callback ){
  var self = this;

  this.nvpreq.execute( 'SetExpressCheckout', params, function ( err, data ){
    if( !err ){
      data.PAYMENTURL = self.make_payment_url( data.TOKEN, path );
    }
